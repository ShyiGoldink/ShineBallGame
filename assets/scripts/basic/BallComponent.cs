...
{
    /// <summary>
    /// 唯一编号。约定：1xxx 移动，2xxx 攻击，3xxx 防御，4xxx 行为，5xxx 形态，6xxx 控制，
    /// 7xxx 咒力（资源：池子这种给别的组件当底子的东西）。
    /// </summary>
    ...
    /// <summary>
    /// **这一招现在能不能用**（默认 true）。出招器（`2005 attack.jujutsu_skill`）在抽招之前
    /// 会挨个问球上的组件，只要有一个人说"用不了"，那一招就被剔出这一轮的抽取——
    /// 而不是放出去空砍一下、白花咒力。
    ///
    /// 认领某一招的组件在这里说自己的条件，比如：
    /// * 领域（`Domain`）：**已经开着、或者正在开（前摇）就别再开了**，熔断期间也不行；
    /// * 灶開（`2009`）：刻印不够 `min_stacks` 就先别放。
    ///
    /// 不认领这一招的组件一律返回 true（不认识就别拦）。
    /// </summary>
    public virtual bool CanUse(string moveId)
    {
        return true;
    }
