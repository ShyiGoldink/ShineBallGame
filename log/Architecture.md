...
10. **领域**：出招挑中 `domain`、或者**对手先开了领域** → 领域组件（`8001`）付账 → 前摇 → 展开（场是施术者的子节点，跟着球走）；生效期间**持有者掉的血**记在外壳上（被护盾吃掉的伤害不算），攒够半血就碎；对方的领域必中则由 `Absorb` 直接磨壳；两个领域碰头时按优先级压缩半径、差 10 以上直接覆盖；收场一律熔断（`burnout_time` 秒）。**被罩住的人**由送过去的 `8002` 判定并定身，能不能还手看有没有 `6002`。
...
| 领域（`8001` 无量空处 + 基类 `Domain`） | ✅ 实机 + 截图验过 | 开法两条（出招随机 / 对手开就跟）、外壳记"**掉了多少血**"（半血碎）+ 对方领域必中直接磨壳、优先级压范围（差 10 覆盖）、收场熔断；半径 800 / 1200，场跟着球走、2px 环是画的、进出用圆心距离 |
...
* **取舍（都在 `FunctionGuide` 的约定里）**：外壳记的是"**持有者实际掉了多少血**"（听 `hp_changed`），
  被护盾/无下限吃掉的不算——所以"能不能打碎领域"取决于有没有穿透（真伤）；
