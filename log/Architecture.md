...
| 咒力那一层 | ✅ 实机通过 | `7001` 池子（咒力 / 恢复 / 熔断 / 黑闪层数与加成表 / 领域状态 / **反转效率的压制系数**）+ `7002` 咒力条；`4005` 反转术式、`2004` 平A（黑闪与熔断分档）、`2005` 出招、`3004` 无下限都站在它上面 |
...
| 领域的效果 | ✅ 实机通过 | `8002` 判定"站在敌方领域里且自己没有领域"→ 定身（`duration` 默认 100000s = 这一局废了）+ **把对面的反转术式效率压成 0**（`reverse_suppress`：中了无量空处用不了反转术式）；`6002` 负责"不能还手" |
...
| 宿儺套件 | ✅ 实机通过 | `7003` 斩击刻印（解/捌每刀 +1；领域必中斩也 +1）→ `2009` 灶開（`2000 + 800×刻印`，实测 14 格 → 13200；刻印不够 5 格时出招器直接不抽它）+ `2007` 解（固定伤害、一次两刀）+ `2008` 捌（`500 + 对手最大血量 8%`）+ `8003` 开放型领域 |
| 招式可用性（`CanUse`） | ✅ 实机通过 | 出招器抽招**之前**问一遍球上的组件：领域开着/前摇中就抽不到 `domain`、灶開刻印不够就抽不到 `fuga`；整轮都用不了就跳过这一次。实测五条 60 秒只展开一次领域、之后全在放苍/赫 |
...
* **已知的粗糙**：① ~~`2005` 不知道"这一招现在能不能用"~~ —— 已做（`BallComponent.CanUse`：
  抽招前先过滤，领域开着就抽不到 `domain`、刻印不够抽不到 `fuga`，整轮都用不了就跳过）；
...
* **还没做的**：宿儺没有防御层（五条三下平A 就把他放倒），这一边的数值还欠一轮；
  `2005` 的"这一招现在能不能用"已经补上了（`BallComponent.CanUse`）。
...
| 加一个组件 | `components/` 下写类（**文件名 = 类名**）→ 有前置组件就在 `Requirements` 里写它的 `Type` 名 → 接线写在 `Bind` 里（注册事件、连信号、改形状）→ 想让某一招"有条件才放"就覆盖 `CanUse` → `ComponentLibrary` 加一行。**装配器不用动** |
