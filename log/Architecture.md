...
| 领域（`8001` 无量空处 + 基类 `Domain`） | ✅ 实机 + 截图验过 | 开法两条（出招随机 / 对手开就跟）；**两条分开的碎裂条件**：① 领域自己的血条（挨打就掉，五条配 24000）② 施术者掉血累计到一半；对方领域必中由 `Absorb` 直接磨壳；优先级压范围（差 10 覆盖）、收场熔断；半径 800 / 1200，场跟着球走、2px 环是画的、进出用圆心距离 |
...
* **取舍（都在 `FunctionGuide` 的约定里）**：领域有**两条命**——自己的血条（挨打就掉，读 `Original`，
  被护盾吃掉的也算）和施术者流血（只听真掉血的那部分），哪条先满都碎；
